    ...
    S: for<'s> System<'s> + Send + 'a,
{
    fn build(self, world: &mut World, builder: &mut DispatcherBuilder<'a, 'b>) -> Result<(), Error> {
        let system_dependencies = self
            ...
            .map(String::as_str)
            .collect::<Vec<&str>>();
        builder.add(self.system_desc.build(world), &self.system_name, &system_dependencies);
        Ok(())
    }
}
...
impl IntegrationTestApplication {
    ...
    pub fn run(self) -> Result<(), Error> {
        ...
        panic::catch_unwind(move || application.lock().expect("Expected to get application lock").run())
        ...
        B: SystemBundle<'static, 'static> + 'static,
    {
        ...
        self.bundle_add_fns
            .push(Box::new(|game_data: CustomGameDataBuilder<'static, 'static>| {
                game_data.with_base_bundle(bundle)
            }));
        self
    }
    ...
    {
        self.bundle_add_fns
            .push(Box::new(move |game_data: CustomGameDataBuilder<'static, 'static>| {
                game_data.with_base_bundle(bundle_function())
            }));
        self
    }
        ...
        Res: Resource,
    {
        ...
        self.resource_add_fns.push(Box::new(move |world: &mut World| {
    ...
    /// Registers a `System` into this application's `CustomGameData`.
    ///
    /// # Parameters
    ///
    ...
    /// Registers a thread local `System` into this application's `CustomGameData`.
    ///
    /// # Parameters
    ///
...
mod test {
    ...
    use super::*;
    use amethyst_test::PopState;

    #[test]
    fn bundle_build_is_ok() -> Result<(), Error> {
        IntegrationTestApplication::blank().with_bundle(BundleZero).run()
    }
    ...
    fn assertion_switch_with_loading_state_with_add_resource_succeeds() -> Result<(), Error> {
        ...
        IntegrationTestApplication::blank().with_state(state_fns).run()
    }
    ...
    fn base_application_can_load_ui() -> Result<(), Error> {
        ...
        IntegrationTestApplication::ui_base::<amethyst::input::StringBindings>()
            .with_assertion(assertion_fn)
            .run()
    }
    ...
    where
        S: State<CustomGameData<'a, 'b>, E> + 'static,
        E: Send + Sync + 'static,
    {
        next_state: Option<S>,
        state_data: PhantomData<dyn State<CustomGameData<'a, 'b>, E>>,
    }
    ...
    impl<'a, 'b, S, E> State<CustomGameData<'a, 'b>, E> for LoadingState<'a, 'b, S, E>
    where
        S: State<CustomGameData<'a, 'b>, E> + 'static,
        E: Send + Sync + 'static,
    {
